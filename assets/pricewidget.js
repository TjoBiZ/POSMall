...
+function ($) { "use strict";
    ...
    mallPrice.prototype.setCurrency = function(currency) {
        ...
        this.$el.trigger('setCurrency.posmall.price', [currency, this.getCurrencyValue(currency)])
    }
    ...
    $.fn.mallPrice = function (option) {
        ...
        this.each(function () {
            var $this   = $(this)
            var data    = $this.data('posmall.price')
            var options = $.extend({}, mallPrice.DEFAULTS, $this.data(), typeof option == 'object' && option)
            if (!data) $this.data('posmall.price', (data = new mallPrice(this, options)))
