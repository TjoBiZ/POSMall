 ...
 */
class AddressForm extends POSMallComponent
{
    ...
    {
        return [
            'name'        => 'kodzero.posmall::lang.components.addressForm.details.name',
            'description' => 'kodzero.posmall::lang.components.addressForm.details.description',
        ];
    }
    ...
    public function defineProperties()
    {
        return [
            'address'  => [
                'type'  => 'dropdown',
                'title' => 'kodzero.posmall::lang.components.addressForm.properties.address.title',
            ],
            'redirect' => [
                'type'  => 'dropdown',
                'title' => 'kodzero.posmall::lang.components.addressForm.properties.redirect.title',
            ],
            'set'      => [
                'type'  => 'dropdown',
                'title' => 'kodzero.posmall::lang.components.addressForm.properties.set.title',
            ],
        ];
    }
    ...
    {
        return [
            'checkout' => trans('kodzero.posmall::lang.components.addressForm.redirects.checkout'),
            'account'  => trans('kodzero.posmall::lang.components.addressForm.redirects.account'),
        ];
    }
    ...
    {
        return [
            null       => trans('kodzero.posmall::lang.common.not_in_use'),
            'billing'  => trans('kodzero.posmall::lang.components.addressForm.set.billing'),
            'shipping' => trans('kodzero.posmall::lang.components.addressForm.set.shipping'),
        ];
    }
    ...
    public function setData()
    {
        ...
        try {
            $this->setVar('address', Address::byCustomer($customer)->where('id', $id)->firstOrFail());
    ...
    public function onSubmit()
    {
        ...
        if ($isNew) {
            ...
            $this->address->customer_id = $customer->id;
        }

        $this->address->fill($data);
        $this->address->customer_id = $customer->id; // enforce ownership after mass-assignment
        ...
        if ($customer->default_shipping_address_id === null) {
            $customer->default_shipping_address_id = $this->address->id;
        }

        if ($customer->default_billing_address_id === null) {
            $customer->default_billing_address_id = $this->address->id;
        }
        $customer->save();
    ...
    protected function getRedirectUrl()
    {
        switch ($this->property('redirect')) {
            case 'payment':
                return $this->controller->pageUrl(GeneralSettings::get('checkout_page'), ['step' => 'payment']);
            case 'quickCheckout':
