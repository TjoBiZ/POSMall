...
class VariantTest extends PluginTestCase
{
    ...
    public function setUp(): void
    {
        ...
        DB::table('system_files')->delete();

        $this->heightProperty = Property::create([
            'name' => 'Height',
            'type' => 'integer',
            'unit' => 'mm',
        ]);

        $this->widthProperty = Property::create([
            'name' => 'Width',
            'type' => 'integer',
            'unit' => 'mm',
        ]);
    ...
    {
        DB::table('kodzero_posmall_product_variants')
    ...
    public function test_name_fallback()
    {
        ...
        $value->property_id = $this->heightProperty->id;
        $value->value       = 200;
        $variant->property_values()->save($value);

        $value              = new PropertyValue();
        $value->product_id  = $product->id;
        $value->property_id = $this->widthProperty->id;
        $value->value       = 400;
        $variant->property_values()->save($value);
