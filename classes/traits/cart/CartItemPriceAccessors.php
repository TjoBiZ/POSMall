...
use KodZero\POSMall\Classes\Totals\TaxTotal;
...
trait CartItemPriceAccessors
{
    ...
    /**
     * The displayed base product price without selected custom field prices.
     */
    public function getBaseProductPostTaxesAttribute()
    {
        return max(0, $this->productPostTaxes - $this->customFieldPostTaxes);
    }

    /**
     * The selected custom field price for a single product.
     */
    public function getCustomFieldPostTaxesAttribute()
    {
        ...
        return $this->custom_field_values->sum(function ($value) use ($currency) {
            return optional($value->priceForFieldOption()->where('currency_id', $currency->id)->first())->integer ?? 0;
        });
    }

    /**
     * The selected custom field price for all quantities.
     */
    public function getTotalCustomFieldPostTaxesAttribute()
    {
        return $this->customFieldPostTaxes * $this->quantity;
    }
    ...
    {
        return $this->servicePreTaxes * $this->serviceQuantityMultiplier;
    }
    ...
    {
        return $this->serviceTaxes * $this->serviceQuantityMultiplier;
    }
    ...
    {
        return $this->servicePostTaxes * $this->serviceQuantityMultiplier;
    }
    ...
    public function getFilteredServiceTaxesAttribute()
    {
        ...
        return $this->service_options->flatMap(function (ServiceOption $option) {
            ...
            $preTaxes = $option->price()->integer / (1 + $factor) * $this->serviceQuantityMultiplier;
    ...
    {
        return optional(optional($this->cart)->shipping_address)->state_id
            ?? optional($this->cart)->getFallbackShippingStateId();
    }

    public function getCartStateCode()
    {
        return optional(optional($this->cart)->shipping_address)->state_code;
    }

    public function getCartZip()
    {
        return optional(optional($this->cart)->shipping_address)->zip;
    }
