...
trait Discounts
{
    ...
    public function applyDiscount(Discount $discount, int $discountCodeLimit = 0)
    {
        ...
        if (in_array($discount->type, $uniqueDiscountTypes)
            ...
            throw new ValidationException([trans('kodzero.posmall::lang.discounts.validation.' . $discount->type)]);
        }
        ...
        if ($discountCodeLimit > 0 && $this->discounts->count() >= $discountCodeLimit) {
            throw new ValidationException([trans('kodzero.posmall::lang.discounts.validation.cart_limit_reached')]);
        }

        if ($this->discounts->contains($discount) || $previousOrderDiscounts->contains($discount->id)) {
            throw new ValidationException([trans('kodzero.posmall::lang.discounts.validation.duplicate')]);
        }

        if ($discount->valid_from && $discount->valid_from->gte(Carbon::now())) {
            throw new ValidationException([trans('kodzero.posmall::lang.discounts.validation.not_found')]);
        }

        if ($discount->expires && $discount->expires->lt(Carbon::today())) {
            throw new ValidationException([trans('kodzero.posmall::lang.discounts.validation.expired')]);
        }

        if ($discount->max_number_of_usages !== null && $discount->number_of_usages >= $discount->max_number_of_usages) {
            throw new ValidationException([trans('kodzero.posmall::lang.discounts.validation.usage_limit_reached')]);
        }
    ...
    public function applyDiscountByCode(string $code, int $discountCodeLimit)
    {
        ...
        if ($code === '') {
            throw new ValidationException([
                'code' => trans('kodzero.posmall::lang.discounts.validation.empty'),
            ]);
        }
        ...
        try {
            ...
            throw new ValidationException([
                'code' => trans('kodzero.posmall::lang.discounts.validation.not_found'),
            ]);
        }
