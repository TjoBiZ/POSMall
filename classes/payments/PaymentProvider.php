...
abstract class PaymentProvider
{
     ...
     * @return string
     */
    public function renderPaymentForm($cartOrOrder, array $variables = []): string
    {
        $formPartial = themes_path(
            "{$this->activeThemeCode()}/partials/posmall/payments/{$this->identifier()}/{$this->paymentFormPartial()}.htm"
        );

        if (! file_exists($formPartial)) {
            $formPartial = plugins_path(
                "kodzero/posmall/classes/payments/{$this->identifier()}/{$this->paymentFormPartial()}.htm"
            );
        }
        ...
        return $template->render(array_merge([
            'cart' => $cartOrOrder,
            'googlePlacesBrowserApiKey' => $this->googlePlacesBrowserApiKey(),
            'googlePlacesAddressAutocompleteEnabled' => $this->googlePlacesAddressAutocompleteEnabled(),
        ], $variables));
    }

    protected function googlePlacesBrowserApiKey(): string
    {
        if (! $this->googlePlacesAddressAutocompleteEnabled()) {
            return '';
        }

        return trim((string)GeneralSettings::getUncached('google_places_browser_api_key', ''));
    }

    protected function googlePlacesAddressAutocompleteEnabled(): bool
    {
        return filter_var(
            GeneralSettings::getUncached('google_places_address_autocomplete_enabled', false),
            FILTER_VALIDATE_BOOLEAN
        ) && trim((string)GeneralSettings::getUncached('google_places_browser_api_key', '')) !== '';
    }
    ...
    {
        $this->order = $order;
        Session::put('posmall.payment.order', optional($this->order)->id);
    ...
    {
        $this->data = $data;
        Session::put('posmall.payment.data', $data);
    ...
    {
        $id = Session::pull('posmall.payment.order');
    ...
    {
        return Session::get('posmall.payment.id');
    }
}
