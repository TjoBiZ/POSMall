...
class Stripe extends PaymentProvider
{
    ...
    public function process(PaymentResult $result): PaymentResult
    {
        ...
        if (! $this->hasConfiguredSecretKey()) {
            return $result->fail(
                ['message' => 'Stripe is not configured for card checkout. Check publishable/secret keys and test mode.'],
                new RuntimeException('Stripe is not configured for card checkout. Check publishable/secret keys and test mode.')
            );
        }
        ...
        if ($response->isRedirect()) {
            ...
            Session::put('posmall.stripe.paymentIntentReference', $response->getPaymentIntentReference());
    ...
    {
        $intentReference     = Session::pull('posmall.stripe.paymentIntentReference');
    ...
    public function settings(): array
    {
        return [
            'stripe_publishable_key' => [
                'label'   => 'kodzero.posmall::lang.payment_gateway_settings.stripe.publishable_key',
                'comment' => 'kodzero.posmall::lang.payment_gateway_settings.stripe.publishable_key_comment',
                'span'    => 'left',
                'type'    => 'text',
            ],
            'stripe_api_key'         => [
                'label'   => 'kodzero.posmall::lang.payment_gateway_settings.stripe.api_key',
                'comment' => 'kodzero.posmall::lang.payment_gateway_settings.stripe.api_key_comment',
                'span'    => 'left',
                'type'    => 'text',
            ],
            'stripe_webhook_secret' => [
                'label'   => 'kodzero.posmall::lang.payment_gateway_settings.stripe.webhook_secret',
                'comment' => 'kodzero.posmall::lang.payment_gateway_settings.stripe.webhook_secret_comment',
                'span'    => 'left',
                'type'    => 'text',
            ]
        ];
    ...
    protected function createCustomerPaymentMethod($customerReference, $cardReference, array $card)
    {
        CustomerPaymentMethod::create([
            'name'              => trans('kodzero.posmall::lang.order.credit_card'),
    ...
    protected function getGateway(): PaymentIntentsGateway
    {
        ...
        $gateway->setApiKey(PaymentGatewaySettings::secret('stripe_api_key'));

        return $gateway;
    }

    protected function hasConfiguredSecretKey(): bool
    {
        $value = PaymentGatewaySettings::secret('stripe_api_key');
