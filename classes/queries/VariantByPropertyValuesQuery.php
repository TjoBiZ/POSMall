...
class VariantByPropertyValuesQuery
{
    ...
    public function query()
    {
        return PropertyValue::leftJoin(
            ...
            'variant_id',
            '=',
            'kodzero_posmall_product_variants.id'
        )
            ->whereNull('kodzero_posmall_product_variants.deleted_at')
            ->where('kodzero_posmall_product_variants.published', true)
            ->where('kodzero_posmall_product_variants.product_id', $this->product->id)
            ->select(DB::raw('variant_id, count(*) as matching_attributes'))
            ->groupBy(['variant_id'])
            ->with('variant')
            ->havingRaw('count(*) = ?', [count($this->ids)])
            ->where($this->subQuery());
    }
