...
namespace KodZero\POSMall\Classes\Downloads;
...
class VirtualProductFileDownload
{
    public function handle(string $key)
    {
        ...
        if ($grant->file) {
            $filename = $this->downloadFilename($grant->display_name, $grant->file->getExtension());
        ...
        if ($product->latest_file && $product->latest_file->file) {
            $filename = $this->downloadFilename($grant->display_name, $product->latest_file->file->getExtension());
        ...
        Log::error(
            '[POSMall] A virtual product without a file attachment has been purchased. You need to fix this!',
    ...
    protected function downloadFilename(string $name, string $extension): string
    {
        $safe = preg_replace('/[^\pL\pN._ -]+/u', '_', $name) ?: '';
        $safe = trim(basename($safe), " .\t\n\r\0\x0B");

        return sprintf('%s.%s', $safe ?: 'download', $extension);
    }
    ...
    {
        Session::put('posmall.login.redirect', Request::url());
        Flash::warning(trans('kodzero.posmall::frontend.session.login_required'));
    ...
    {
        return trans('kodzero.posmall::lang.product_file.errors.' . $key);
    }
}
