...
{
    public $table = 'kodzero_posmall_product_file_grants';
    ...
    public function getDownloadLinkAttribute(): string
    {
        ...
        return URL::to('/posmall/download/' . $encodedKey);
    }
        ...
        if ($this->order_product->product->inventory_management_method == 'single') {
            ...
            $productFiles = ProductFile::whereNotExists(function (QueryBuilder $query) {
                $query->select(DB::raw(1))
                    ->from('kodzero_posmall_product_file_variant')
                    ->whereColumn(
                        'kodzero_posmall_product_file_variant.product_file_id',
                        'kodzero_posmall_product_files.id'
                    );
            })
                ->where('kodzero_posmall_product_files.product_id', '=', $this->order_product->product->id)
                ->get();
    ...
    public static function fromOrderProduct(OrderProduct $orderProduct)
    {
        ...
        for ($i = 0; $i < $orderProduct->quantity; $i++) {
            ...
            Event::fire('posmall.product.file_grant.created', [$grant, $orderProduct->product]);
