...
class ImageSet extends Model
{
    ...
    public const MORPH_KEY = 'posmall.imageset';
    ...
    public $table = 'kodzero_posmall_image_sets';
    ...
    {
        $existingSets = DB::table('kodzero_posmall_image_sets')
        ...
        if ($this->is_main_set) {
            DB::table('kodzero_posmall_image_sets')
                ->where('product_id', $this->product_id)
                ->where('id', '<>', $this->id)
