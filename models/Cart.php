...
use KodZero\POSMall\Classes\Cart\DiscountApplier;
...
use KodZero\POSMall\Classes\Traits\Cart\CartActions;
use KodZero\POSMall\Classes\Traits\Cart\CartSession;
use KodZero\POSMall\Classes\Traits\Cart\Discounts;
...
class Cart extends Model
{
    ...
    public const FALLBACK_SHIPPING_COUNTRY_KEY = 'posmall.fallback_shipping_country_id';
    public const FALLBACK_SHIPPING_STATE_KEY = 'posmall.fallback_shipping_state_id';

    public $rules = [];

    public $table = 'kodzero_posmall_carts';
    ...
    public $belongsToMany = [
        'discounts' => [
            Discount::class,
            'table' => 'kodzero_posmall_cart_discount',
        ],
    ];
    ...
    public function setQuantity($cartProductId, int $quantity)
    {
        ...
        if ($product) {
            ...
            Event::fire('posmall.cart.product.quantityChanged', [$product, (int)$oldQuantity, (int)$quantity]);
        }
    ...
    public function getMatchingProductInCart(Product $product, ?Variant $variant = null, ?Collection $values = null): CartProduct|null
    {
        ...
        $expectedKeys = $this->customFieldValueComparisonKeys($values);

        return CartProduct::with('custom_field_values')
            ->whereIn('id', $ids)
            ->get()
            ->first(function (CartProduct $cartProduct) use ($expectedKeys) {
                return $this->customFieldValueComparisonKeys($cartProduct->custom_field_values) === $expectedKeys;
            });
    }

    protected function customFieldValueComparisonKeys(Collection $values): array
    {
        return $values->map(function ($value) {
            $optionId = $value->custom_field_option_id === null
                ? 'null'
                : (string)$value->custom_field_option_id;

            return implode(':', [
                (string)$value->custom_field_id,
                $optionId,
                (string)$value->value,
            ]);
        })->sort()->values()->all();
    ...
    public function getFallbackShippingStateId()
    {
        return Session::get(self::FALLBACK_SHIPPING_STATE_KEY);
    }
    ...
    public function setFallbackShippingStateId($value)
    {
        if (is_scalar($value) && ctype_digit((string)$value)) {
            Session::put(self::FALLBACK_SHIPPING_STATE_KEY, $value);
        }
    }
    ...
    public function forgetFallbackShippingCountryId()
    {
        ...
        Session::forget(self::FALLBACK_SHIPPING_STATE_KEY);
    }
    ...
    {
        Session::put('posmall.shipping.enforced.' . $id . '.price', $price);

        if ($name) {
            Session::put('posmall.shipping.enforced.' . $id . '.name', $name);
        }
    }
    ...
    {
        Session::forget('posmall.shipping.enforced');
    }
