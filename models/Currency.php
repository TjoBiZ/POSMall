...
class Currency extends Model
{
    ...
    public const CURRENCY_SESSION_KEY = 'posmall.currency.active';

    public const CURRENCIES_CACHE_KEY = 'posmall.currencies';

    public const DEFAULT_CURRENCY_CACHE_KEY = 'posmall.currency.default';

    public const JSON_PRICE_CACHE_KEY = 'posmall.jsonPrice.currencies';
     ...
     * @var string
     */
    public $table = 'kodzero_posmall_currencies';
     ...
     */
    public $rules = [
        'code'          => 'required|unique:kodzero_posmall_currencies,code',
        'rate'          => 'bail|required|numeric',
        'decimals'      => 'bail|required|integer|min:0',
        'format'        => 'required',
    ...
    {
        if (static::$allCurrencies !== null) {
            return static::$allCurrencies;
        }

        return static::$allCurrencies = Currency::hydrate(Cache::rememberForever(static::CURRENCIES_CACHE_KEY, fn () => Currency::get()->keyBy('id')->toArray()));
    }
    ...
    {
        if (!static::requestHasSessionCookie()) {
            return static::defaultCurrency();
        }
    ...
    protected static function requestHasSessionCookie(): bool
    {
    ...
    {
        Log::error(
            '[POSMall] Unknown currency was requested',
            ['currency' => $currency, 'url' => request()->url()]
        );

        return new self([
            'code'       => '???',
            'symbol'     => trans('kodzero.posmall::lang.currency_settings.unknown'),
            'rate'       => 1,
            'decimals'   => 2,
    ...
    {
        DB::table('kodzero_posmall_prices')->where('currency_id', $this->id)->delete();
        DB::table('kodzero_posmall_product_prices')->where('currency_id', $this->id)->delete();
        DB::table('kodzero_posmall_customer_group_prices')->where('currency_id', $this->id)->delete();
        Cache::forget(self::CURRENCIES_CACHE_KEY);
        Cache::forget(self::DEFAULT_CURRENCY_CACHE_KEY);
        Cache::forget(self::JSON_PRICE_CACHE_KEY);
        static::$allCurrencies = null;
