...
class PropertyValue extends Model
{
    ...
    public $table = 'kodzero_posmall_property_values';
    ...
    public function getDisplayValueAttribute()
    {
        ...
        if ($this->isColor()) {
            ...
            return sprintf(
                ...
                e($this->sanitizeColorHex((string)($value['hex'] ?? ''))),
                e((string)($value['name'] ?? ''))
            );
        }
        ...
        if ($type === 'checkbox') {
            ...
            return trans('kodzero.posmall::lang.common.' . $key);
        }

        return e($value);
    }

    public function getSafeColorHexAttribute(): string
    {
        $value = $this->isColor() ? $this->getValueAttribute() : [];

        return $this->sanitizeColorHex((string)($value['hex'] ?? ''));
    }
    ...
    private function sanitizeColorHex(string $hex): string
    {
        $hex = trim($hex);

        return preg_match('/^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/', $hex) ? $hex : 'transparent';
    }
}
