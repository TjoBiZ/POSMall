...
class Variant extends Model
{
    ...
    public const MORPH_KEY = 'posmall.variant';
     ...
     * @var string
     */
    public $table = 'kodzero_posmall_product_variants';
    ...
    public $belongsToMany = [
        'files' => [
            ProductFile::class,
            'table' => 'kodzero_posmall_product_file_variant',
        ],
    ];
    ...
    {
        DB::table('kodzero_posmall_property_values')->where('variant_id', $this->id)->delete();
        DB::table('kodzero_posmall_wishlist_items')->where('variant_id', $this->id)->delete();
    }

    public function getImageSetIdOptions()
    {
        $null = [
            '' => '-- ' . trans('kodzero.posmall::lang.image_sets.create_new'),
        ];
    ...
    public function getAttribute($attribute)
    {
        ...
        if (app()->runningInBackend()) {
            $inheritanceDisabled = session()->get('posmall.variants.disable-inheritance');
        } else {
    ...
    public static function resolveItem($item, $url, $theme)
    {
        ...
        $items = self::published()
            ->whereHas('product', function ($query) {
                $query->where('kodzero_posmall_products.published', true);
            })
            ->with('product')
            ->get()
