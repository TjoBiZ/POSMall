...
use KodZero\POSMall\Classes\Traits\Images;
use KodZero\POSMall\Classes\Traits\PDFMaker;
use KodZero\POSMall\Classes\Traits\PriceAccessors;
use KodZero\POSMall\Classes\Traits\ProductPriceAccessors;
use KodZero\POSMall\Classes\Traits\PropertyValues;
use KodZero\POSMall\Classes\Traits\StockAndQuantity;
use KodZero\POSMall\Classes\Traits\UserSpecificPrice;
...
class Product extends Model
{
    ...
    public const MORPH_KEY = 'posmall.product';
     ...
     * @var string
     */
    public $table = 'kodzero_posmall_products';
    ...
    public $hasMany = [
        ...
        'warehouse_inventory'     => [WarehouseInventory::class],
    ];
    ...
    public $belongsToMany = [
        ...
        'accessories'     => [
            Product::class,
            'table'      => 'kodzero_posmall_product_accessory',
            'key'        => 'accessory_id',
            'otherKey'   => 'product_id',
        ...
        'vendors'         => [
            Vendor::class,
            'table'    => 'kodzero_posmall_product_vendor',
            'key'      => 'product_id',
            'otherKey' => 'vendor_id',
        ],
        'channels'        => [
            Channel::class,
            'table'    => 'kodzero_posmall_product_channel',
            'key'      => 'product_id',
            'otherKey' => 'channel_id',
        ],
    ];
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->bindEvent('model.relation.beforeAttach', function ($relationName, $attachedIdList, $insertData) {
            ...
            $this->flushApiCatalogCacheForRelation($relationName);
        });
    ...
    protected function flushApiCatalogCacheForRelation(string $relationName): void
    {
        if (in_array($relationName, ['categories', 'brand', 'vendors', 'channels', 'warehouse_inventory', 'image_sets', 'prices'], true)) {
    ...
    public function afterDelete()
    {
        ...
        DB::table('kodzero_posmall_product_accessory')->where('product_id', $this->id)->delete();
        DB::table('kodzero_posmall_product_tax')->where('product_id', $this->id)->delete();
        DB::table('kodzero_posmall_cart_products')->where('product_id', $this->id)->delete();
        DB::table('kodzero_posmall_product_custom_field')->where('product_id', $this->id)->delete();
        DB::table('kodzero_posmall_category_product')->where('product_id', $this->id)->delete();
        DB::table('kodzero_posmall_wishlist_items')->where('product_id', $this->id)->delete();
    }

    public function getYoutubeEmbedsAttribute(): array
    {
        return collect($this->embeds ?: [])
            ->map(function ($embed) {
                $code = (string)($embed['code'] ?? $embed['url'] ?? '');
                $videoId = self::extractYoutubeVideoId($code);
                ...
                return [
                    'title'         => $title !== '' ? $title : 'Product video',
                    'video_id'      => $videoId,
    ...
    public static function extractYoutubeVideoId(?string $value): ?string
    {
        ...
        if (preg_match('~clip_id=["\']([A-Za-z0-9_-]{6,32})~i', $value, $matches)) {
            return $matches[1];
        }

        if (preg_match('~(?:youtu\.be/|youtube(?:-nocookie)?\.com/(?:embed/|v/|shorts/))([A-Za-z0-9_-]{6,32})~i', $value, $matches)) {
            return $matches[1];
        }

        $parts = parse_url(html_entity_decode($value, ENT_QUOTES));
        if (!is_array($parts) || empty($parts['host']) || empty($parts['query'])) {
            return null;
        }

        if (!preg_match('~(^|\.)youtube(?:-nocookie)?\.com$~i', $parts['host'])) {
            return null;
        }
        ...
        return preg_match('~^[A-Za-z0-9_-]{6,32}$~', $videoId) ? $videoId : null;
    }
    ...
    public function scopeWithoutServiceCarriers($query)
    {
    ...
    public function scopeServiceCarriers($query)
    {
        return $query->where('user_defined_id', 'like', 'POSMALL-SERVICE-CARRIER-%');
    }
    ...
    {
        return ['' => trans('kodzero.posmall::lang.common.none')]
    ...
    {
        return Cache::rememberForever(self::sortOrderCacheKey($this->id), fn () => DB::table('kodzero_posmall_category_product')
    ...
    {
        return [
            'single'  => 'kodzero.posmall::lang.variant.method.single',
            'variant' => 'kodzero.posmall::lang.variant.method.variant',
        ];
    }
